...
class DriverHistoricalCompletedBookings:
    ...
    #     self.save_path = os.path.join(store.processed_data_dir, 'driver_history_data.csv')

    def __call__(self, df: pd.DataFrame) -> pd.DataFrame:
        driver_history_df = store.get_processed("driver_history_data.csv")
    ...
    def build(self, df: pd.DataFrame) -> None:
        ...
        store.put_processed("driver_history_data.csv", driver_day_events, header=True)
